...
def crop(img_path, mask_path, img_save_path, mask_save_path):
    ...
    for axis in [0, 1, 2]:
        ...
        # Remove those layers
        img_arr = np.delete(
            img_arr, list(range(remove_front_index - 10)) + list(range(remove_back_index + 10, len(sums))),
            axis=(axis + 1) % 3
        )

        mask_arr = np.delete(
            mask_arr, list(range(remove_front_index - 10)) + list(range(remove_back_index + 10, len(sums))),
            axis=(axis + 1) % 3
        )
        print(" -> ", img_arr.shape, end=" ")
...
if __name__ == '__main__':
    ...
    mask_path = r'H:\PRM\59_cases_nii\59_cases_nii_i_lungmask'
    img_save_path = r'H:\PRM\59_cases_nii\registration_e2i_lungbox'
    mask_save_path = r'H:\PRM\59_cases_nii\59_cases_nii_i_lungmask_lungbox'
