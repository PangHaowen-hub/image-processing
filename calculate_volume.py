...
def volume(mask):
    ...
    # return lobe1, lobe2, lobe3, lobe5


if __name__ == '__main__':
    mask_path = r'G:\Lobectomy\LLL_nii\LL_before_test_all'
    mask_list = get_listdir(mask_path)
    mask_list.sort()
    workbook = xlwt.Workbook(encoding='utf-8')  # 创建一个workbook 设置编码
    worksheet = workbook.add_sheet('LU_before')  # 创建一个worksheet
    for i in tqdm.trange(len(mask_list)):
        lobe1, lobe2, lobe3, lobe4, lobe5 = volume(mask_list[i])
        # lobe1, lobe2, lobe3, lobe5 = volume(mask_list[i])
    ...
    workbook.save(r'G:\Lobectomy\LLL_nii\LL_before.xls')  # 保存
